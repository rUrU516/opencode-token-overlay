...
- 内置 72 个可轮换角色，每跑完 100,000 Token 自动切换下一个
...
角色通过 `characters/character-framework.js` 注册，统一支持 `idle / run / resist / launch / tumble / land` 阶段。落地后的 `disperse / assemble` 像素切换由框架统一处理，角色无需重复实现。首批 10 个角色位于 `characters/roster.js`，第二批 12 个位于 `characters/roster-wave2.js`，第三批 50 个跨动物、机械、植物、食物、神话、物件、交通与抽象生物题材的角色位于 `characters/roster-wave3.js`；新增角色的接口与示例见 [`characters/README.md`](characters/README.md)。脚本加载和注册顺序就是每轮结束后的切换顺序。
