...
- 内置 22 个可轮换角色，每跑完 100,000 Token 自动切换下一个
...
角色通过 `characters/character-framework.js` 注册，统一支持 `idle / run / resist / launch / tumble / land` 阶段。落地后的 `disperse / assemble` 像素切换由框架统一处理，角色无需重复实现。首批角色位于 `characters/roster.js`，第二批位于 `characters/roster-wave2.js`；新增角色的接口与示例见 [`characters/README.md`](characters/README.md)。脚本加载和注册顺序就是每轮结束后的切换顺序。
